      ...
      <b-row class="g-4">
        ...
        <b-col cols="12" md="3" class="col-left d-flex flex-column align-items-center">
          ...
          <section class="feedbacks-card w-100 mt-4 rounded shadow-sm d-flex flex-column">
            ...
            <div class="px-3 pb-3 mt-auto">
              ...
              />
              <b-button size="sm" variant="primary" class="w-100" @click="adicionarFeedback">
                Adicionar
              </b-button>
            </div>
          </section>
        ...
        <b-col cols="12" md="9" class="col-right">
          ...
          <section class="bloco-descricao rounded mb-4 p-3">
            ...
            <div v-if="isAdmin" class="editor-wrapper rounded overflow-hidden">
              <quill-editor
            ...
            <!-- NÃO ADMIN: Apenas texto -->
            <div v-else class="descricao-readonly-wrapper rounded">
              <div
                class="descricao-readonly bg-white text-dark"
                v-html="perfil.descricao_html || '<p>Descrição ainda não cadastrada.</p>'"
              />
            </div>
          </section>
              ...
              <div class="cursos-grid p-3">
                ...
                >
                  ...
                  <div
                    ...
                    :class="{ 'text-success': curso.concluido, 'text-primary': !curso.concluido }"
                  >
                    {{ curso.nome }}
                  </div>
...
import { authState } from '@/core/services/auth.service.js'
...
const isAdmin = computed(() => {
  ...
  const permissoes = Array.isArray(u.permissoes)
    ? u.permissoes.map(p => p.toString().toLowerCase())
    : []

  const tipo = (u.tipo_usuario || u.tipo || '').toString().toLowerCase()

  const adminFlag = Boolean(u.is_admin || u.admin || false)
...
const percentualConcluido = computed(() =>
  cursos.value.length ? Math.round((cursosConcluidos.value / cursos.value.length) * 100) : 0
)
...
async function fetchCursosDepartamento(slug) {
  if (!slug) return
  const r = await api.get('/api/v1/cursos', {
    params: { departamento_slug: slug, apenas_ativos: true }
  })
...
async function salvarDescricao() {
  ...
  try {
    ...
    if (meId.value) {
      try {
        await api.patch(`/api/v1/usuarios/${meId.value}`, payload)
      } catch {}
    }
  }
}
...
async function salvarCursos() {
  const progresso = cursos.value.map(c => ({
    slug: c.slug,
